...
from config.pakistan_jobs_config import SOURCE_PRIORITY


def generate_fingerprint(title, company, city):
    """Normalize and hash title+company+city for O(1) exact lookup."""
...
def handle_duplicate(db: Session, new_job: Dict, existing_job: Job, source_priority=None) -> Job:
    """Merge a duplicate into the existing record and keep the best version."""
